...
import java.time.{ZoneOffset, OffsetTime}
...
class GlobalClock(startTime: OffsetTime) extends Subject{
  private var time = startTime.withOffsetSameInstant(ZoneOffset.UTC)
  ...
  def removeObserver(o: Observer): Unit=
  {
    observers =observers.filterNot(_==0)
  }
  def notifyObservers(): Unit=
  {
...
class LocalClock(val timeZone: ZoneOffset, globalClock: GlobalClock) extends Observer {
   ...
   var time =OffsetTime.of(0,0,0,0,timeZone)
   globalClock.registerObserver(this)

  def update(thsTime:OffsetTime)={
    time = thsTime.withOffsetSameInstant(timeZone)
  }
  override def toString() ={
     time.toString()
  }
