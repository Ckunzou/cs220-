...
object MyBuilder extends Builder {
  ...
  private[markov] def buildWithLines(lines: Iterator[String],
    ...
    // TODO: implement this one!
    {
      if(!lines.hasNext)
        ...
        {
          val tup = buildWithWords(splitWords(lines.next()),prefix,chain)
          buildWithLines(lines,tup._1,tup._2)
        }
    }
  ...
  private[markov] def buildWithWords(words: Array[String],
    ...
    // TODO: implement this one!
    {
      if(words.isEmpty)
        ...
        {
          val firstWord = new Suffix(getFirstWord(words))
          if(chain.keySet.contains(prefix))
          {
            buildWithWords(getRestOfWords(words),prefix.shiftIn(words(0)),addToChain(chain,prefix,firstWord))
          }
          else
          {
            buildWithWords(getRestOfWords(words),prefix.shiftIn(words(0)),chain+(prefix-> Vector(firstWord)))
          }
  ...
  private[markov] def addToChain(chain: Chain, prefix: Prefix, suffix: Suffix): Chain =
    // TODO: implement this one!
    {
       chain.updated(prefix, addSuffix(chain(prefix), suffix))
    }
}
...
object MyGenerator extends Generator {
    ...
    // TODO: implement this one!
    {
      ...
      if(words==0)
      {
       s
        ...
        val vecSuff = getSuffixes(chain,prefix)
        val ranSuff = getRandomSuffix(vecSuff,rand)
        if(isSuffixNewline(ranSuff))
         s
        else
          {
            val result1 = generateAll(chain,words-1,rand,prefix.shiftIn(ranSuff.toString))
            val result = ranSuff.toString + " " + result1
            result
          }
  ...
  private[markov] def getRandomSuffix(suffixes: Vector[Suffix], rand: Randomizer): Suffix =
    // TODO: implement this one!
    {
     val ranNum= rand.random(getSuffixesLength(suffixes))
      suffixes(ranNum)
    }
