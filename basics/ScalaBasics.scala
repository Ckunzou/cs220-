...
object ScalaBasics {
  ...
  def isPalindrome(s: String): Boolean = {
    ...
    for (i <- 0 to halfLen; j <- halfLen to 0 if (i != j)) yield false
    true
  ...
  def wordCounter(lines: Array[String]): Map[String, Int] =
  {
    ...
    for (eachStr <- lines)
    {
      val str = eachStr.toLowerCase;
      for (words <- str.split(" "))
      {
        val numWord = result.getOrElse(words, 0);
        result += (words -> (numWord + 1));
      }
