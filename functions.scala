...
object EA03 {
    ...
    def tail[A](ls: List[A]) :List[A] = ls match {
        case Nil => throw new IllegalArgumentException("illegal argument")
        case l::xs => xs
    }
    ...
    def drop[A](ls:List[A], n: Int) :List[A]= (ls,n) match {
        case (l:List[A],x:Int) if(l.length<=x )=> Nil
        case _ =>ls.takeRight(ls.length - n)
    }
    ...
    def length[A](ls:List[A]):Int={
        foldLeft(ls,0)((x,_)=>x+1)
    }
    def reverse[A](ls:List[A]):List[A]={
        foldLeft(ls,List[A]())((x,y)=>y::x)
    }
    def flatten[A](ls:List[List[A]]):List[A] = {
        foldLeft(ls,List[A]())((x,y)=>x++y)
    }
    ...
    def maxAverage(ls:List[(Double,Double)]): Double= {
           ...
           val max =  foldLeft(ls,List[Double]()){(x,y)=>
              if(y._1>y._2)
                  x.+:(y._1)
              else
                  x.+:(y._2)
